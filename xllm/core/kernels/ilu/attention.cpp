...
void reshape_paged_cache(const at::Tensor &key,
    ...
    TORCH_CHECK(scalar_type == at::ScalarType::Half || scalar_type == at::ScalarType::BFloat16);
    ...
    // this kernel is only used in vllm, key is always contiguous on dim [1,2], and so should v.
    // But v is not contiguous on model deepseek, wihch is split form tensor: [b, num_heads, k + v], k is ready for used by using mla kernels.
    // so add a value head stride here to store k..
    ...
    // TORCH_CHECK(value.size(2) == head_size); # support value head_dim != key head_dim for deepseek(128 and 192)

    // num_blocks, num_heads, block_size, head_size
    TORCH_CHECK(key_cache.dim() == 4);
    TORCH_CHECK(value_cache.dim() == 4);

    // translate kvcache shape from [n_blocks, block_size, n_heads, head_dim] to (num_blocks, num_heads, block_size, head_size)
    ...
    TORCH_CHECK(key_cache.numel() == num_blocks * num_heads * block_size * head_size);
    TORCH_CHECK(value_cache.numel() == num_blocks * num_heads * block_size * head_size);// still use head_size, but we only fill value_head_stride
    ...
    if (scalar_type == at::ScalarType::Half) {
        kernels::infer::vllm_reshape_and_cache(
                ...
                (half *) key_cache.data_ptr(), (half *) value_cache.data_ptr(),
                (const int64_t *) slot_mapping.data_ptr(), key_token_stride, value_token_stride, value_head_stride, num_heads,
                head_size, value_head_size, block_size, num_tokens, stream);
    } else {
        kernels::infer::vllm_reshape_and_cache(
                (const __nv_bfloat16 *) key.data_ptr(), (const __nv_bfloat16 *) value.data_ptr(),
                (__nv_bfloat16 *) key_cache.data_ptr(), (__nv_bfloat16 *) value_cache.data_ptr(),
                (const int64_t *) slot_mapping.data_ptr(), key_token_stride, value_token_stride, value_head_stride, num_heads,
                head_size, value_head_size, block_size, num_tokens, stream);
    }
}
...
void batch_prefill( torch::Tensor& query,
    ...
    auto query_start_loc_ = query_start_loc.value_or(torch::Tensor());
    auto kv_start_loc_ = kv_start_loc.value_or(torch::Tensor());
    ...
    // auto alibi_slope_ = alibi_slope.value_or(torch::Tensor());
...
size_t get_paged_attention_workspace(int num_seqs, int num_heads, int num_kv_heads,
    ...
    std::cout<<num_seqs<<","<<num_heads<<","<< num_kv_heads<< ","<<head_size<< ","<<block_size<< ","<<max_context_len<<std::endl;
    CUINFER_CHECK(cuInferPageAttentionGetWorkspaceV7(num_seqs, num_heads, num_kv_heads, head_size, block_size, max_context_len, &workspace_size));
...
at::Tensor vllm_paged_attention(
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    size_t workspace_size = get_paged_attention_workspace(num_seqs, num_heads, num_kv_heads, head_size, block_size, max_context_len);
    ...
    kernels::desc::paged_attention(out_desc, query_desc, key_cache_desc, value_cache_desc, num_kv_heads, scale,
                                   block_tables_desc, context_lens_desc, block_size, max_context_len, alibi_slopes_desc,
                                   causal, window_left, window_right, softcap, enable_cuda_graph, use_sqrt_alibi, sinks_desc, stream, workspace_ptr);
    return out;
}
